...
export default function DelegateModal({ isOpen, onDismiss, title }: VoteModalProps) {
  ...
  const kswapBalance = useTokenBalance(account ?? undefined, chainId ? KSWAP[chainId] : undefined)
      ...
      {!attempting && !hash && (
        <ContentWrapper gap="lg">
          <AutoColumn gap="lg" justify="center">
            ...
            <TYPE.body>Earned KSWAP tokens represent voting shares in Kswap governance.</TYPE.body>
            <TYPE.body>
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOndismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.main fontSize={36}>{kswapBalance?.toSignificant(4)}</TYPE.main>
          </AutoColumn>
        </LoadingView>
      )}
      {hash && (
